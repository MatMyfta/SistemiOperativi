...
struct p_state {
  pid_t pid_p_owner; // Process P that owns the actual state
  list *q_list;
  unsigned int m;
};
  ...
  while (1) {
    if (getline(&message, &message_size, fin) >= 0) {
      ...
      if (!strcmp(command.command, UNITNOS_P_COMMAND_STATUS)) {
        list_node *node_q;
        unitnos_q **q;
        list_for_each_data(q, unitnos_q *, node_q, state.q_list) {
          log_info("Q PID %u, created by P PID %u\n",unitnos_q_get_pid(*q),state.pid_p_owner);          
        }        
      }
    } else if (feof(fin)) {
