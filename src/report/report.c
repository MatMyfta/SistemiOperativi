...
static void unitnos_report_record_print(int *unitnos_report_records,
                                        int position, int total_char) {
  float percentage = 0;
  if (total_char != 0)
    percentage =
        (float)((float)(unitnos_report_records[position] * 100.0) / total_char);
  printf("|%9s |%10d|%9.3lf%|\n", get_character(position),
         unitnos_report_records[position], percentage);
  printf("+----------+----------+----------+\n");
}
